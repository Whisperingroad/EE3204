...
float str_cli(FILE *fp, int sockfd, long *len)
{
	...
	int previousMsgAcknowledgement = TRUE;
  ...
  /*** the whole file is loaded in the buffer. ***/
	...
	{	
		...
		if (previousMsgAcknowledgement == FALSE || previousMsgAcknowledgement == NACK ){
		 	printf("previous message is not acknowledged\n");
			ci -= slen;
			previousMsgAcknowledgement = TRUE;
		}

		if (previousMsgAcknowledgement){
			...
			if (acknowledgeNumber == 1)
				...
				acknowledgeNumber = 1;
			ci += slen;
			previousMsgAcknowledgement = FALSE;
		}
		...
		if (ack.num != acknowledgeNumber || ack.len != slen){

			if (slen == DATALEN){
				printf("error in transmission\n");
				printf("retranmission is required\n");
			}
			//Complete sending
			else
			{
				printf("No more messages to be sent\n");
			}
